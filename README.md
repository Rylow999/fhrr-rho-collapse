...
5. **Universality across algebras.** The ρ=1 transition holds quantitatively in HRR real (gram 0.05, pinv 0.13, pure 0.98; Exp V3), in BSC binary (Exp 11b: gram 0.214 / pinv 0.244 / pure 0.988 at the square point with κ_med = 4.7×10³ in the critical band), and **MAP refines the law** (Exp 11c): in MAP the Gram decoder is single-shot least squares, so the critical band κ = 1.06×10⁴ at ρ=1 does NOT kill it (0 errors are not re-injected); MAP collapses instead at ρ>1 (rank-deficient: gram 0.07–0.24, pinv ≈ 1.0). Two-factor picture: you need κ in the band AND a closed loop to get the collapse. Plus LiDAR voxelization (gram 1.0→0.33). The κ-window transfers across algebras; whether it *fires* depends on the observer's dynamics.
