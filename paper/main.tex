...
\textbf{Ensemble generality (Exp.~20A)}. The same intervention across six codebook families at $\rho=1$: Gaussian ($0.160 \to 0.979$), Rademacher ($0.128 \to 0.986$), uniform-on-sphere ($0.164 \to 0.983$), and Toeplitz-correlated ($0.175 \to 0.947$) all replicate; an orthogonal frame (QR factor of a Gaussian matrix; tight at $n=d$) does not collapse (the ambient operator is undefined there rather than unstable); a deliberately pathological near-duplicate frame breaks all decoders, as expected.
