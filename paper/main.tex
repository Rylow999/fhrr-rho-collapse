...
\paragraph{Limitations.} The random-matrix anchoring (Sec.~\ref{sec:rmt}) provides the asymptotic scaling and identifies the ensemble, but our $n=32$ is far from the asymptotic regime; for $\rho < 1$ the measured $\lambda_{\min}$ sits $\mathcal{O}(10^{2})$ above the asymptotic Marchenko--Pastur edge, a finite-size offset we report rather than fit away. We tested FHRR, HRR-real, BSC, and MAP quantitatively; sparse block codes and other bind operators are open. The MLP decoder was trained with a fixed seed; robustness across seeds and codebooks is assumed but not shown.
