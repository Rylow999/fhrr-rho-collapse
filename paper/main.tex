...
\begin{abstract}
The phase diagram identifies where resonator-based decoding is stable and where it fails; the strongest causal claim lives at the square point, where the Frame-Dual Stability Principle pins down both the failure and its repair in spectral terms. Three regimes emerge: (I)~$\rho > 1$, over-complete (rank-deficient, structurally singular) Gram matrices; (II)~$\rho = 1$, the square point, where the Gram matrix is critically conditioned and ambient Gram decoders collapse; (III)~$\rho < 1$, under-complete frames which are well conditioned. We anchor regime~II in random-matrix theory: at $\rho=1$ the lower Marchenko--Pastur edge reaches zero and the square Wishart enters its \emph{hard-edge} regime, with $\lambda_{\min} \sim n^{-2}$ and condition number $\kappa \sim 4n^{2}$ scaling, consistent with measurements over $n \in [32, 512]$. The mechanism is then established by a controlled causal intervention (Exp.~18): at $\rho=1$ with identical codebook, Gram matrix, resolvent, initial states and facts, swapping only the wiring from $M^{-1}$ (ambient) to $C^{\top} M^{-1} C$ (dual, same resolvent) moves decoding from $0.16$ to $0.99$, while the operator norm drops from $1/\lambda_{\min}$ to exactly $1$. The collapse depends on the observer's operator wiring --- we state this as the Frame-Dual Stability Principle (Prop.~\ref{prop:fdl}), supported by independent reimplementation, six codebook ensembles, BSC binary codes, FHRR causal replication, and a four-operator zoo.
\end{abstract}
...
\paragraph{Scaling in $n$: the square-Wishart prediction holds.} Equation~\eqref{eq:kappa-scaling} is a sharp, falsifiable prediction: moving to larger codebooks at $\rho=1$ should scale $\kappa$ as $4n^{2}$, with $\lambda_{\max} \to 4$ and $\lambda_{\min}$ collapsing as $n^{-2}$. Exp.~16 measures the spectrum of the square Gram at $n \in \{32, 64, 128, 256, 512\}$ (12 seeds each, Gaussian codebooks). The measured medians are consistent with $O(n^{2})$ hard-edge scaling (ratios $1.8$--$12.9$ against the nominal $4n^{2}$, within the expected spread of the heavy-tailed Wishart condition number). The finite-$n$ offset reported in Sec.~\ref{sec:rmt} ($n=32$, ratio $1.76$) is the small-$n$ end of this scaling, not an anomaly.
...
The $\rho$-phase diagram reveals that \emph{capacity} in VSA is not a fixed property but depends on the decoder class. The ``binary collapse'' appears across the tested ambient Gram-inverse decoder configurations, but not for resonator decoders that avoid hard-edge inversion. This suggests that universal laws in distributed representations must be stated relative to observer classes, not as absolute properties of the space --- concretely, the testable content of this claim is the hard-edge ambient-resolvent condition (Def.~\ref{def:closure}).
