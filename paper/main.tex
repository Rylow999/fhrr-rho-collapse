...
The $\rho = 1$ singular point corresponds not to a geometric transition but to the \emph{condition number} $\kappa(M)$ entering a numerically dangerous regime where the decoder fails catastrophically (this is the pattern-level observation; the per-seed analysis in Sec.~\ref{sec:kappa-mech} refines it to a mechanism, showing the strict window is not a necessary condition). The observed pattern:
...
\begin{proposition}[Frame-Dual Stability Principle]
\label{prop:fdl}
Let $C \in \mathbb{R}^{n\times d}$ be a codebook with Gram $M = CC^{\top}$ and let $T$ be an operator acting on the coefficient space. Write $A = C$ (analysis), $S = C^{\top}$ (synthesis). The ambient operator $T$ acting on the state has $\|T\|_2$ possibly large; the dual operator $S T A = C^{\top} T C$ is the orthogonal projector onto $\operatorname{row}(C)$ whenever $T = M^{-1}$, and its pseudo-inverse analogue $C^{\top} M^{+} C$ is an orthogonal projector in all cases. In particular $\|C^{\top} M^{-1} C\|_2 \le 1$ with equality iff $C$ has full column rank $d$. The critical decoding failure at $\rho=1$ therefore is a property of the decoder's \emph{operator placement}, not of the frame itself.
\end{proposition}
...
\item \textbf{Operator zoo} (DONE). Exp.~23: $M^{+}$, Tikhonov ($\lambda=10^{-3}, 10^{-1}$), truncated SVD in both wirings. The dual gain equals $1$ exactly for the projective operators ($M^{-1}$, $M^{+}$); regularized variants (Tikhonov, truncated SVD) sit slightly below one because they deliberately shrink the weak eigendirection --- the dual is stable for every operator tested, and exact for the two projective constructions.
...
\paragraph{Limitations.} The RMT anchoring is verified across $n \in [32, 512]$ (Exp.~16). The Frame-Dual Stability Principle is supported by independent reimplementation (Exp.~19), six codebook ensembles including pathological cases (Exp.~20, Exp.~25), the BSC algebra (Exp.~18b), a de-VSA-ified linear pipeline (Exp.~21), dynamics tracing (Exp.~22), a four-operator zoo plus frame-bounds sweep (Exps.~23, 24), and an empirical stability bound for generic $T$ (Exp.~26). Open: FHRR-complex causal test (our Exp.~18 used HRR-real), and a formal proof of the bound class for general unbounded $T$ beyond the projective case.
