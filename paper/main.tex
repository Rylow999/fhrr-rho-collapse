...
\textbf{Binary Spatter Codes (BSC).} Preliminary tests with a BSC resonator (Exp.~V11, \texttt{src/exp\_V11\_bsc\_binary.py}) show the same qualitative structure --- the Gram-inverse decoder is the only observer class that degrades at $\rho=1$ --- but the measured accuracies depend strongly on the binarisation cleanup and we do not treat them as quantitative confirmation. The $\rho$-law for binary algebras is stated here as a conjecture supported by the shared algebraic mechanism (block-wise Gram inversion), pending the dedicated binary sweep identified in \texttt{docs/INFORME\_TRANSFERENCIA.md}.
...
\paragraph{Limitations.} The random-matrix anchoring (Sec.~\ref{sec:rmt}) provides the asymptotic scaling and identifies the ensemble, but our $n=32$ is far from the asymptotic regime; for $\rho < 1$ the measured $\lambda_{\min}$ sits $\mathcal{O}(10^{2})$ above the asymptotic Marchenko--Pastur edge, a finite-size offset we report rather than fit away. The binary-algebra claim (BSC) is preliminary and qualitative only; a dedicated binary sweep is future work. We tested FHRR and HRR-real quantitatively; MAP and sparse block codes are open. The MLP decoder was trained with a fixed seed; robustness across seeds and codebooks is assumed but not shown.
